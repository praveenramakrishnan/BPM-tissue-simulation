    ...
    for iloop_radius = 1:num_radius_sphere
        ...
        if recompute_mie_solution
            ...
            [efield_mie_x, efield_mie_y, efield_mie_z] = compute_mie_solution( ...
             ...
             efield_propagated_mie = efield_mie_x(:, :, end);
             save(filename_mie_efield_output, 'efield_propagated_mie', 'x_grid', 'y_grid', '-v7.3');
        ...
        imagesc(1e6*x_grid, 1e6*y_grid, squeeze(abs(efield_propagated_mie)));
        xlabel('x ($\mu$m)', 'interpreter', 'latex');
        ylabel('y ($\mu$m)', 'interpreter', 'latex');
        title("Electric field magnitude (V/m)");
        colorbar;
        ...
        hold on;
        plot(1e6*x_grid, abs(efield_propagated_mie(:, find(y_grid==0))), 'DisplayName', 'Mie');
        hold off;
        xlabel('x ($\mu$m)', 'interpreter', 'latex');
        ylabel('|E| (V/m)');
        ...
        hold on;
        plot(1e6*y_grid, abs(efield_propagated_mie(find(x_grid==0), :)), 'DisplayName', 'Mie');
