function [efield_propagated] = run_beam_propagation(efield_initial, num_grid_points_x, num_grid_points_y, ...
    ...
    if use_gpu_arrays
        efield_propagated = gpuArray(efield_propagated);
        refractive_index_data = gpuArray(refractive_index_data);
        frequency_x_grid_nd = gpuArray(frequency_z_grid_nd); 
    end
    ...
    if use_gpu_arrays
        efield_propagated = gather(efield_propagated);
        refractive_index_data = gather(refractive_index_data);
        frequency_x_grid_nd = gather(frequency_z_grid_nd); 
        z_grid_refractive_index = gather(z_grid_refractive_index); 
    end
end
